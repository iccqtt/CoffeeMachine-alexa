...
def convertText(input_text):
	#WARNING! This function requires FFMPEG to be installed!
	print("Generating speech to: "+input_text)
	tts = gTTS(input_text, lang='en')
	tts.save('custom.mp3')
	print("Generated MP3 file!")
	print("Converting MP3 to WAV...")
	sound = AudioSegment.from_mp3("custom.mp3")
	sound.export("custom.wav", format='wav')
	subprocess.call(['ffmpeg','-nostats','-hide_banner','-y', '-i', 'custom.mp3', '-acodec', 'pcm_s16le', '-ar', '16000', 'custom.wav'])
	print("Playing back query...")
	subprocess.call(['aplay', 'custom.wav'])
	print("Sending query to Alexa...")
	generatedAudio = open('custom.wav', 'r')
	alexa_speech_recognizer(generatedAudio)
...
def sendAlexaText(message):
	global isSendText
	if message == '1':
		audioOn = open('turn_on.wav', 'r')
		#subprocess.call(['aplay', 'turn_on.wav'])
		isSendText = TRUE
		alexa_speech_recognizer(audioOn)

	elif message == '0':
		audioOff = open('turn_off.wav', 'r')
		isSendText = TRUE
		#subprocess.call(['aplay', 'turn_off.wav'])
		alexa_speech_recognizer(audioOff)
